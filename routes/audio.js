  ...
  app.post("/api/audio", async (req, res) => {
    ...
    try {
      ...
      if (error.status >= 300 && error.status <= 400) {
        console.log("Request successfull. Status code " + error.status);

        sendSuccess();
      } else {
        const errorStatement = `An audio feed parsing error occurred : ${error.message}
        )}`;
