const buildRssContainer = (feed, fullElementClassName, callback) => {
  ...
  const retrieveLinks = length => {
    for (let i = 0; i < length; i++) {
      ...
      if (i === length - 1 && (Number((length - 1).toString().split('')[0]) > 1)) {
        ...
        let arrowsContainer = $('<span></span>')
          ...
          .prepend(leftArrow)
          .append(`<span>1 / ${(length - 1).toString().split('')[0]}</span>`)
          .append(rightArrow)
  ...
  if (feed.length >= 50) retrieveLinks(50)
  else retrieveLinks(feed.length);
}
